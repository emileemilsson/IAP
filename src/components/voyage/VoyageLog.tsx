...
import { VoyageAuto } from './VoyageAuto';
...
React.useEffect(() => {
      if (STTApi.voyAutoRecall && voyage && voyage.state === 'recalled') {
         download('narrative.' + STTApi.playerData.character.voyage[0].id + '.json',
            JSON.stringify(voyageExport),
            'Export voyage narrative JSON',
            'Export',
            false,
            '/home/local/CITD/paul.bilnoski/Documents/stt/narratives/');
      }
   }, [voyageExport]);
   ...
   async function choose(index: number) {
      ...
      let voy = STTApi.playerData.character.voyage[0];
      if (!voy || !voy.id || !voy.dilemma || !voy.dilemma.id) {
         return;
      }
      await resolveDilemma(voy.id, voy.dilemma!.id, index);

      // Remove the dilemma that was just resolved
      voy.dilemma = undefined;
